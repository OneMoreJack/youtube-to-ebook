...
为了提高处理速度、减少 API 调用成本并避免 YouTube 的频率限制，本项目实现了两级缓存：

1.  **字幕缓存 (`transcripts/`)**:
    - 首次获取视频字幕后，会以 `{video_id}.txt` 的形式保存在本地。
    - 再次处理同一视频时，直接从本地读取，不再请求 YouTube。
2.  **文章缓存 (`articles/`)**:
    - Claude AI 生成的文章会以 `{video_id}.md` 的形式保存在本地。
    - **性能优化**：如果检测到文章已缓存，系统将**直接跳过**获取字幕和调用 AI 的步骤，极大地节省了时间和 Token。

> [!TIP]
> 如果你想强制重新生成文章（例如更改了 Prompt），可以手动删除 `articles/` 文件夹下对应的 `.md` 文件。
